    ...
    public class PosBot<T> : ActivityHandler where T : Dialog
    {
        ...
        protected override async Task OnMembersAddedAsync(IList<ChannelAccount> membersAdded, ITurnContext<IConversationUpdateActivity> turnContext, CancellationToken cancellationToken)
        {
            foreach (var member in membersAdded)
            {
                ...
                {
                    await turnContext.SendActivityAsync(MessageFactory.Text($"Hola soy el asistente virtual de la Escuela Militar de Ingeniería Posgrado Unidad Académica Santa Cruz, dime como puedo ayudarte."), cancellationToken);
                }
            }
        }
